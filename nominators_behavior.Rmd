...
To summarize the data, we can clearly say that nominators rarely engage with their nomination after their initial action to become a nominator. On average, nominators changed their bonded amount only `r round((mean(remove_1kv_nominators(overall_nominators_active)$nr_amount_changed)-1), digits = 2)` times and `r round((mean(remove_1kv_nominators(overall_nominators_active)$nr_targets_changed)-1), digits =2)` times after the first time. While there are some much more active nominators, this is a strong indication that a large share of nominators simply set and forget. Additional to the fact that many nominations are very old (), there is room for improvement and the data supports initiatives suggested as in RFC XX (TODO).
