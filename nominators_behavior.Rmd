---
title: "State of Nominator Activity and Validator Backing on Polkadot"
...
## Special Thanks
We would like to express our gratitude to the Parity Data team for their continuous efforts in maintaining the data sources that make analyses like this possible. In particular, a special thanks goes to Pranay for providing clean and well-structured datasets, which have been invaluable for this work.
...
In the data, the bonded amount of a nominator increases either if it was a concious choice by the nominator (either bonding extra or withdrawing bond) or if the reward destination was set to compounding. Only tracking changes in the bonded amount would therefore inflate the number by the fact that many nominators use the compounding feature. To tackle this issue, we define a manual increase in the bonded amount if it exceeded a (extrapolated) `r APY*100`% APR. In other words, we assume that the increase was due to a payout of staking rewards if it is the size of a usual staking reward relative to the bond size. Sampling different nominators manually shows that this approximation is working well.
...
The two graphs above indicate that most nominators, after their initialization, never actively engage with their target selection nor the bonded amount again.
...
Given the data above, we can say, for example, that `r round(result$value,digit = 1)*100`% of the density is above `r threshold` days (or around `r round(threshold/30, digits = 1)` months). In other words, `r round(result$value,digit = 1)*100`% of the total backing of all validators is older than `r round(threshold/30.5, digits = 1)` months.
...
The analysis presented here has shown that many Nominators simply set their targets and never really engage with their nominations (at least expressed as an on-chain change) again. This leads to the predictable result that the average **Weighted Backing Age** of Validators is rather large with `r round(mean(validator_staleness_summary$weighted_avg_days_since_change), digits = 0)`. This does not necessarily mean that the current set of Validators is not optimal. But finding ways to gain more confidence that Nominators are up to date with the recent developments in the Validator set is desirable. RFC (TODO) proposes some mechanism to achieve this, while not making it too difficult for Nominators.