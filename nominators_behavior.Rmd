...
overall_nominators_active <- overall_nominators %>%
  ...
  mutate(
    staleness_level = case_when(
      days_since_target_changed <= 14 ~ 1,   # Level 1: 0-14 days
      days_since_target_changed >= 15 & days_since_target_changed <= 30 ~ 2,  # Level 2: 15-30 days
      days_since_target_changed >= 31 & days_since_target_changed <= 90 ~ 3,  # Level 3: 31-90 days
      days_since_target_changed >= 91 & days_since_target_changed <= 180 ~ 4, # Level 4: 91-180 days
      days_since_target_changed >= 181 ~ 5,  # Level 5: 181+ days
      TRUE ~ NA_real_  # Handle cases where days_since_target_changed is NA
    )
  )
...
validator_nominators_merged <- validator_stakers_filtered %>%
  left_join(overall_nominators_active[, c("stash_address", "days_since_target_changed", "staleness_level")], 
            by = "stash_address")
# Keep only the relevant columns for analysis
validator_nominators_staleness <- validator_nominators_merged %>%
  select(validator_address, stash_address, bonded_amount, staleness_level, days_since_target_changed)
