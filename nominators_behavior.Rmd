...
initiate_data <- function(df_t0, df_t1, threshold) {
  # Merge the data frames by stash_address
  ...
  # Vectorized logic to identify bonded_amount_changed based on the new condition
  merged_df$bonded_amount_changed <- mapply(function(b0, b1, s0, s1, df_t1_session) {
    # Case when the old session is missing and the new session exists
    if (is.na(s0) && !is.na(s1)) {
      return(list(s1))  # New entry, track the new session
    # Case when the old session exists but the new session is missing
    } else if (!is.na(s0) && is.na(s1)) {
      return(list(c(s0, df_t1_session)))  # Old entry dropped, track the session
    # Case when the balance increases and we need to check the threshold
    ...
    # Case when the balance decreases, always trigger a change
    } else if (!is.na(b0) && b1 < b0) {
      return(list(c(s0, s1)))  # Balance decreased, track the session
    # Case when no change has occurred
    } else {
...
update_data <- function(initial_data, new_data, threshold) {
  ...
  # Merge the initial and new data frames by stash_address (left join)
  merged_df <- merge(initial_data, new_data, by = "stash_address", all.x = TRUE, suffixes = c("_old", "_new"))

  # Vectorized update of targets_changed using mapply
  merged_df$targets_changed <- mapply(function(t_old, t_new, s_old, s_new, changed_list, new_session) {
    if (!is.na(s_old) && !is.na(s_new)) {
      if (t_old != t_new) {
        # Ensure we don't duplicate session entries in changed_list
        if (!(s_new %in% changed_list)) {
          return(append(changed_list, s_new))
        } else {
          return(changed_list)
        }
      } else {
  ...
  # Vectorized update of bonded_amount_changed using mapply
  merged_df$bonded_amount_changed <- mapply(function(b_old, b_new, s_old, s_new, changed_list, new_session) {
    if (!is.na(s_old) && !is.na(s_new)) {
      if (b_new > b_old * threshold) {
        ...
        if (!(s_new %in% changed_list)) {
          return(append(changed_list, s_new))  # Balance decreased, always trigger change
        } else {
          return(changed_list)
        }
      } else {
