...
ggplot(remove_1kv_nominators(overall_nominators), aes(x = bonded_changed_percent)) +
...
In this section, we analyze in greater detail how the backing of Validators is composed.
...
```{r, echo = FALSE}
hist(validator_staleness_summary$weighted_avg_days_since_change)
```

```{r, echo = FALSE}
# TODO: Merge the validator addresses with the names, sort by staleness and give the names
...
head(tmp %>%
       select(name, weighted_avg_days_since_change))
```